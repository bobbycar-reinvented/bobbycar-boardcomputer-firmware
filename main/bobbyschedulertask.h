...
public:
    ...
    BobbySchedulerTask(const char *name, void (&setupCallback)(), void (&loopCallback)(),
                       espchrono::millis_clock::duration loopInterval, bool use_in_recovery = false, bool init_later = false,
    ...
    {
        if (m_wasInitialized && !force)
            return;

        if (m_init_later && !in_recovery && !force)
            return;
    ...
    bool hasDelayedInit() const { return m_init_later; }
private:
