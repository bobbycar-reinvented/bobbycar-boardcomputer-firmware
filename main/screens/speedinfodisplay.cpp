...
void SpeedInfoDisplay::redraw(espgui::TftInterface &tft)
{
    ...
    m_labelSpeed.redraw(tft, font,
                        ...
                        (std::abs(avgSpeedKmh) < 100 ? fmt::format("{:.1f}", avgSpeedKmh) :
    ...
    if (const auto avgVoltage = controllers.getAvgVoltage(); avgVoltage)
    {
        ...
        m_voltageLabel.redraw(tft, font, fmt::format("{:.1f} V", avgVoltage.value()), espgui::TFT_WHITE,
                              espgui::TFT_BLACK, 4);
        font.setTextSize(2);
    ...
    m_distanceLabel.redraw(tft, font,
                           ...
                           fmt::format("{:.3f} km", drivingStatistics.meters_driven / 1000) :
                           (drivingStatistics.meters_driven > 100 ?
                            fmt::format("{:.1f} m", drivingStatistics.meters_driven) :
                            fmt::format("{:.2f} m", drivingStatistics.meters_driven)), espgui::TFT_WHITE,
                           espgui::TFT_BLACK, 4);
